...
desc "setup dummy app for testing (drops dev & test database"
...
task :install_engine do 
  ...
  system("cd spec/dummy && rails g common_engine:install")
  
  # copy over a recent version of the schema since CommoneEngine does not have
  # migrations at the moment.
  FileUtils.cp("spec/support/schema.txt",  "spec/dummy/db/schema.rb")
  
  system("cd spec/dummy && rake db:schema:load")
  system("cd spec/dummy && rake db:test:prepare")
end
