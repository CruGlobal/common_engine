class Team < ActiveRecord::Base
  ...
  has_many :team_members, -> { includes(:person).order(Person.table_name + ".lastName") }, :foreign_key => "teamID"
  has_many :people, -> { order(Person.table_name + ".lastName") }, :through => :team_members
  has_many :activities, -> { includes(:target_area).order(TargetArea.table_name + ".name") }, :foreign_key => 'fk_teamID', :primary_key => "teamID"
